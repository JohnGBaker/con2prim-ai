...
#else
                                                                     , CCTK_REAL *S_entropy_local, CCTK_REAL *S_star_local ) {
#endif
...
#if( USE_ENTROPY_EQUATION )
  // Update S_entropy_local in case we had pressure floors and/or ceilings
  *S_entropy_local = U[PRESSURE] * pow(U[RHOB],1.0-eos.Gamma_ppoly_tab[polytropic_index]);
#endif
...
#if( USE_ENTROPY_EQUATION )
  *S_star_local     = alpha_sqrt_gamma * (*S_entropy_local) * uUP[0];
#endif
